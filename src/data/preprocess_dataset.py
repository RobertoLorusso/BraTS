...
#from preprocess.graphgen import img2graph
...
STANDARDIZATION_STATS = ([0.4645, 0.6625, 0.4064, 0.3648], [
...
class DataPreprocessor():
    def __init__(self, args):
        ...
        self.data_dir = os.path.expanduser(
            args.data_dir if args.data_dir else os.getenv("INPUT_MRI_DIR"))
        self.output_dir = os.path.expanduser(
            args.output_dir if args.output_dir else f"{os.getenv('PROCESSED_DATA_DIR')}")  # _{args.num_nodes}_{args.boxiness}_{args.num_neighbors}
        ...
        data_stats = self.compute_dataset_stats(
        ) if STANDARDIZATION_STATS is None else STANDARDIZATION_STATS
    ...
    def get_all_mris_in_dataset(self):
        ...
        scan_dic = {os.path.split(fp)[0].split(
            "/")[-1]: fp for fp in mri_folders}
    ...
    def get_standardized_image(self, scan_full_path):
        image_data = nifti_io.read_in_patient_sample(
            scan_full_path, self.modality_extensions)

        if(self.label_extension):
            label_data = nifti_io.read_in_labels(
                scan_full_path, self.label_extension)
            standardized_labels = swap_labels_from_brats(label_data)
        else:
            standardized_labels = None

        normalized_data = normalize_img(image_data)
        standardized_data = standardize_img(
    ...
    def compute_dataset_stats(self):
        ...
        for mri_id in self.all_ids:
            ...
            img = nifti_io.read_in_patient_sample(
                mri_path, self.modality_extensions)
            lab = nifti_io.read_in_labels(mri_path, self.label_extension)
            if(len(img.shape) > 3):
                healthy_tissue_mask = np.logical_and(
                    img[:, :, :, 0] > 0.001, lab == 0)
    ...
    def process_next_sample(self, mri_id):
        ...
        image_data, label_data = self.get_standardized_image(
            self.id_to_fp[mri_id])
        image_data = image_data.T

        # with open(f"{self.output_dir}{os.sep}{mri_id}_crop.pkl", "wb") as f:
        #    pickle.dump(crop_idxs, f)
        # !------------------------------
        # ! IMG TO GRAPH AND SAVE TO DISK
        # !-----------------------------
        ...
        #    pickle.dump(crop_idxs, f)
        return mri_id, image_data

    def run(self):
        ###### REMOVE INDEX SELECTION WHEN CREATING FUTURES ##########
        with concurrent.futures.ThreadPoolExecutor(max_workers=N_THREADS) as executor:
            futures = [executor.submit(self.process_next_sample, mri_id)
                       for mri_id in self.all_ids[0:50]]
            print("Set up Threads, starting execution")
            for future in concurrent.futures.as_completed(futures):
                try:
                    mri_id, image_data = future.result()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data_dir', default=None,
                        help='path to the directory where data is stored', type=str)

    parser.add_argument('-o', '--output_dir', default=None,
                        help='Directory to save graphs to', type=str)
    parser.add_argument('-m', '--modality_extensions', nargs="+", default=["_flair.nii.gz", "_t1.nii.gz", "_t1ce.nii.gz", "_t2.nii.gz"],
                        help="The file extensions for each desired modality. Accepts a variable amount of modalities. Ensure consistent order.")
    parser.add_argument('-l', '--label_extension', default=None,
                        help='What the label extension is. If not provided will ignore labels (i.e. do not provide when preprocessing evaluation data)')
    parser.add_argument('-p', '--data_prefix', default="",
                        help='A prefix that all data folders share, i.e. BraTS2021.')
